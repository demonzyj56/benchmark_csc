#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Script for running test on city/gray dataset."""
import argparse
import datetime
import logging
import pickle
...
def parse_args():
    ...
    parser.add_argument('--output_path', default='.city.gray.noise_free', type=str, help='Path for output')
    parser.add_argument('--dataset', default='city', type=str, help='Dataset to use')
    parser.add_argument('--lambda', '--lmbda', dest='lmbda', default=0.2, type=float, help='Lambda value for CDL')
    parser.add_argument('--cfg', default='experiments/city.gray.yml', type=str, help='Config yaml file for solvers')
...
def load_OCSC_dict(args):
    ...
    pattern = re.compile(r'city\.gray\.[0-9]+\.mat')
...
def load_OCSC_time_stats(args):
    ...
    path = os.path.join(args.output_path, 'OCSC', 'city.gray.time_stats.mat')
    assert os.path.exists(path)
...
def main():
    ...
    if (not args.no_legacy) and \
        ...
        os.system('ln -s /backup/OCSC/result/city_10/ {:s}'.format(
            os.path.join(args.output_path, 'OCSC')
        ))
    log_name = os.path.join(
        args.output_path,
        '{:s}.{:s}.{:%Y-%m-%d_%H-%M-%S}.test.log'.format(
            args.name, 'city.gray', datetime.datetime.now(),
        )
    )
    ...
    test_blob = get_dataset('city', train=False, gray=True)
